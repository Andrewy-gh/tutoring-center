...
import { sessionDataService, type StudentProgressHistory } from './sessions-service';
...
export default async function SingleSessionPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const session = await sessionDataService.getSession(sessionId);
  ...
  const progressHistory: StudentProgressHistory[] = canViewProgressHistory
    ? await sessionDataService.getStudentRecentProgress(session.student.id, session.id, 5)
    : [];
            ...
            >
              <div>
                ...
                <Link
                  ...
                  href={`/dashboard/students/${session.student.id}`}
                >
