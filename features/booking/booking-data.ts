import 'server-only';
import { forbidden } from 'next/navigation';
import { getTutorOptionRowsByIds, type TutorOptionRow } from '@/db/queries/tutors';
import { getCurrentParentCredits } from '@/features/credits/server';
import { getStudents } from '@/features/students/students-service';
import { getSubjects } from '@/features/subjects/subjects-service';
import type { UserRole } from '@/lib/auth';
import { startOfDay } from 'date-fns';
import { z } from 'zod';
import type { StudentOption, TutorOption } from './types';

const TUTOR_OPTIONS_ERROR_MESSAGES = {
  database: 'Tutor options are temporarily unavailable. Please retry in a moment.',
  validation: 'Tutor options format is invalid. Please try again later.',
} as const;

const WeekDaySchema = z.enum(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday']);
const AvailabilityRowSchema = z.object({
  week_day: WeekDaySchema,
  start_time: z.string(),
  end_time: z.string(),
});

const TutorOptionJoinRowSchema = z.intersection(
  z.object({
    id: z.number(),
    userId: z.number(),
    education: z.string().nullable(),
    yearsExperience: z.number().nullable(),
    firstName: z.string().nullable(),
    lastName: z.string().nullable(),
    email: z.string(),
    phone: z.string().nullable(),
  }),
  z.union([
    z.object({
      weekDay: z.null(),
      startTime: z.null(),
      endTime: z.null(),
    }),
    z.object({
      weekDay: WeekDaySchema,
      startTime: z.string(),
      endTime: z.string(),
    }),
  ])
);

const TutorOptionJoinRowListSchema = z.array(TutorOptionJoinRowSchema);

const TutorOptionQueryRowSchema = z.object({
  id: z.number(),
  user_id: z.number(),
  education: z.string().nullable(),
  years_experience: z.number().nullable(),
  first_name: z.string().nullable(),
  last_name: z.string().nullable(),
  availability: z.array(AvailabilityRowSchema),
});

const TutorOptionQueryRowListSchema = z.array(TutorOptionQueryRowSchema);

type TutorOptionJoinRow = z.infer<typeof TutorOptionJoinRowSchema>;
type TutorOptionQueryRow = z.infer<typeof TutorOptionQueryRowSchema>;
type WeekDay = z.infer<typeof WeekDaySchema>;

const WEEKDAY_ORDER: Record<WeekDay, number> = {
  Monday: 1,
  Tuesday: 2,
  Wednesday: 3,
  Thursday: 4,
  Friday: 5,
  Saturday: 6,
  Sunday: 7,
};

const WEEKDAY_SHORT: Record<WeekDay, string> = {
  Monday: 'Mon',
  Tuesday: 'Tue',
  Wednesday: 'Wed',
  Thursday: 'Thu',
  Friday: 'Fri',
  Saturday: 'Sat',
  Sunday: 'Sun',
};

const MISSING_VALUE = '—';

const formatTime = (time: string) => {
  const match = /^(\d{1,2}):(\d{2})/.exec(time);
  if (!match) return time;

  const hour24 = Number.parseInt(match[1], 10);
  const minute = match[2];
  if (!Number.isFinite(hour24) || hour24 < 0 || hour24 > 23) return time;

  const hour12 = hour24 % 12 === 0 ? 12 : hour24 % 12;
  const suffix = hour24 >= 12 ? 'PM' : 'AM';
  if (minute === '00') {
    return `${hour12}${suffix}`;
  }

  return `${hour12}:${minute}${suffix}`;
};

const toAvailabilitySummary = (availability: TutorOptionQueryRow['availability']) => {
  const rows = availability.slice().sort((left, right) => {
    const weekdayCompare = WEEKDAY_ORDER[left.week_day] - WEEKDAY_ORDER[right.week_day];
    if (weekdayCompare !== 0) return weekdayCompare;
    return left.start_time.localeCompare(right.start_time);
  });

  if (rows.length === 0) return null;

  const chunks = rows.map(
    row => `${WEEKDAY_SHORT[row.week_day]} ${formatTime(row.start_time)}-${formatTime(row.end_time)}`
  );
  if (chunks.length <= 2) return chunks.join(' • ');
  return `${chunks.slice(0, 2).join(' • ')} +${chunks.length - 2} more`;
};

const mapTutorOption = (tutor: TutorOptionQueryRow): TutorOption => ({
  id: tutor.id,
  user_id: tutor.user_id,
  name: [tutor.first_name, tutor.last_name].filter(Boolean).join(' ').trim() || MISSING_VALUE,
  education: tutor.education,
  years_experience: tutor.years_experience,
  typicalAvailability: toAvailabilitySummary(tutor.availability),
});

const groupTutorOptions = (rows: TutorOptionJoinRow[]) => {
  const tutorsById = new Map<number, TutorOptionQueryRow>();

  for (const row of rows) {
    const existingTutor = tutorsById.get(row.id);

    if (existingTutor) {
      if (row.weekDay !== null) {
        existingTutor.availability.push({
          week_day: row.weekDay,
          start_time: row.startTime,
          end_time: row.endTime,
        });
      }

      continue;
    }

    tutorsById.set(row.id, {
      id: row.id,
      user_id: row.userId,
      education: row.education,
      years_experience: row.yearsExperience,
      first_name: row.firstName,
      last_name: row.lastName,
      availability:
        row.weekDay === null
          ? []
          : [
              {
                week_day: row.weekDay,
                start_time: row.startTime,
                end_time: row.endTime,
              },
            ],
    });
  }

  return Array.from(tutorsById.values());
};

function toStudentOption(student: Awaited<ReturnType<typeof getStudents>>[number]): StudentOption {
  return {
    id: student.id,
    name: student.name || 'N/A',
    grade: student.grade,
  };
}

export async function getTutorOptionsByIds(role: UserRole, tutorIds: number[]): Promise<TutorOption[]> {
  if (role !== 'parent' && role !== 'admin') {
    forbidden();
  }

  const uniqueTutorIds = [...new Set(tutorIds.filter(id => Number.isInteger(id) && id > 0))];
  if (uniqueTutorIds.length === 0) {
    return [];
  }

  let rawRows: TutorOptionRow[];
  try {
    rawRows = await getTutorOptionRowsByIds(uniqueTutorIds);
  } catch {
    throw new Error(TUTOR_OPTIONS_ERROR_MESSAGES.database);
  }

  const parsedJoinRows = TutorOptionJoinRowListSchema.safeParse(rawRows);
  if (!parsedJoinRows.success) {
    throw new Error(TUTOR_OPTIONS_ERROR_MESSAGES.validation);
  }

  const parsedTutors = TutorOptionQueryRowListSchema.safeParse(groupTutorOptions(parsedJoinRows.data));
  if (!parsedTutors.success) {
    throw new Error(TUTOR_OPTIONS_ERROR_MESSAGES.validation);
  }

  const sortOrder = new Map(uniqueTutorIds.map((id, index) => [id, index]));

  return parsedTutors.data
    .map(mapTutorOption)
    .sort(
      (left, right) =>
        (sortOrder.get(left.id) ?? Number.MAX_SAFE_INTEGER) - (sortOrder.get(right.id) ?? Number.MAX_SAFE_INTEGER)
    );
}

export async function getNewSessionBookingData(role: UserRole) {
  const [students, subjects, parentCredits] = await Promise.all([
    getStudents(role),
    getSubjects(role),
    getCurrentParentCredits(),
  ]);
  const tutorIds = subjects.flatMap(subject => subject.assignments.map(assignment => assignment.tutorId));
  const tutors = await getTutorOptionsByIds(role, tutorIds);
  const today = startOfDay(new Date());

  return {
    parentId: parentCredits.parentId,
    initialBalance: parentCredits.balance,
    students: students.map(toStudentOption),
    subjects,
    tutors,
    todayStartMs: today.getTime(),
  };
}
