...
const existingTypeAssertionEntries = [
  ...
  'lib/db/book-session.ts::(await tx.execute(sql` insert into sessions ( tutor_id, student_id, subject_id, parent_id, slot_units, scheduled_at, ends_at, status ) values ( ${input.tutorId}, ${input.studentId}, ${input.subjectId}, ${input.parentId}, ${input.slotUnits}, ${input.scheduledAt}::timestamptz, ${input.endsAt}::timestamptz, ${status} ) returning id, tutor_id, student_id, subject_id, parent_id, slot_units, scheduled_at, ends_at, status `)) as Array<{ id: number; tutor_id: number; student_id: number; subject_id: number; parent_id: number; slot_units: number; scheduled_at: string; ends_at: string; status: SessionStatus; }>',
  'lib/db/book-session.ts::(await tx.execute(sql` select id from credit_balances where parent_id = ${input.parentId} limit 1 `)) as Array<{ id: number }>',
  "lib/db/book-session.ts::(await tx.execute(sql` select id from sessions where tutor_id = ${input.tutorId} and ${overlapStatusPredicate()} and tstzrange(scheduled_at, ends_at, '[)') && tstzrange(${input.scheduledAt}::timestamptz, ${input.endsAt}::timestamptz, '[)') limit 1 `)) as Array<{ id: number }>",
  'lib/db/book-session.ts::(await tx.execute(sql` select id from students where id = ${input.studentId} and parent_id = ${input.parentId} limit 1 `)) as Array<{ id: number }>',
  'lib/db/book-session.ts::(await tx.execute(sql` update credit_balances set available_minutes = available_minutes - ${sessionMinutes}, pending_minutes = pending_minutes + ${sessionMinutes}, updated_at = now() where parent_id = ${input.parentId} and available_minutes >= ${sessionMinutes} returning available_minutes, pending_minutes `)) as Array<{ available_minutes: number; pending_minutes: number }>',
  'lib/db/book-session.ts::db as BookSessionDatabase',
  'lib/db/client.ts::globalThis as typeof globalThis & { __tutoringCenterSql?: ReturnType<typeof postgres>; __tutoringCenterDb?: AppDatabase; }',
  "lib/db/queries/credits/balances.ts::(await import('@/lib/db/client')).db as unknown",
  "lib/db/queries/credits/balances.ts::(await import('@/lib/db/client')).db as unknown as SqlExecutor",
  ...
  'tests/lib/db/book-session.test.ts::(draft.balance ? [{ id: 1 }] : []) as T',
  'tests/lib/db/book-session.test.ts::(draft.overlap ? [{ id: 7001 }] : []) as T',
  'tests/lib/db/book-session.test.ts::(draft.studentOwned ? [{ id: input.studentId }] : []) as T',
  'tests/lib/db/book-session.test.ts::[] as T',
  'tests/lib/db/book-session.test.ts::[] as T',
  'tests/lib/db/book-session.test.ts::[draft.balance] as T',
  'tests/lib/db/book-session.test.ts::[session] as T',
  'tests/lib/db/credit-balances.test.ts::response as T',
];
